...
| LGT8F328P    | 32 КБ  | 2 КБ   | 0/1/2/4/8 КБ<br>(настраивается, разделяется с FLASH) | 2 командных слова |
...
| Устройство   | FLASH  | E2PROM |
|--------------|--------|--------|
| LGT8F328P    | 32 КБ  | 0 КБ   |
|              | 30 КБ  | 1 КБ   |
|              | 28 КБ  | 2 КБ   |
|              | 24 КБ  | 4 КБ   |
|              | 16 КБ  | 8 КБ   |
...
| EEARH: 0x22 (0x42) | Значение по умолчанию: 0x0000 |
|---|---|
| EEARL: 0x21 (0x41) |    |
...
| 14:8 | EEARH | Старшие 7 бит адреса доступа к EFLASH/E2PROM.|
| 7:0 | EEARL | Младшие 8 бит адреса доступа к EFLASH/E2PROM.|
...
|---------------------|--------------------------------------------|
| EEDR/E2PD0: 0x20 (0x40) | Значение по умолчанию: 0x00                |
| Биты                | EEDR[7:0]                                  |
| R/W                 | R/W                                        |

#### Описание битов
|        | EEDR     | Регистр данных E2PCTL                     |
|--------|----------|-------------------------------------------|
| [7:0]  | E2PD0    | В 16/32-битном режиме используется для хранения младшего байта |
...
|---------------------|-------------------------------------------|
| E2PD1: 0x5A         | Значение по умолчанию: 0x00               |
| Биты                | E2PD1[7:0]                                |
| R/W                 | R/W                                       |
...
|        | E2PD1    |
|--------|----------|
| [7:0]  | В 16-битном режиме: хранит старшие 8 бит 16-битных данных. |
|        | В 32-битном режиме: хранит старшие 8 бит младших 16 бит данных. |

---

### E2PD2 - Регистр данных 2

|                     | E2PD2 — Регистр данных FLASH 2            |
|---------------------|-------------------------------------------|
| E2PD2: 0x57         | Значение по умолчанию: 0x00               |
| Биты                | E2PD2[7:0]                                |
| R/W                 | R/W                                       |
...
|        | E2PD2    |
|--------|----------|
| [7:0]  | В 32-битном режиме: хранит младшие 8 бит старших 16 бит данных. |

---

### E2PD3 - Регистр данных 3

|                     | E2PD3 — Регистр данных FLASH 3            |
|---------------------|-------------------------------------------|
| E2PD3: 0x5C         | Значение по умолчанию: 0x00               |
| Биты                | E2PD3[7:0]                                |
| R/W                 | R/W                                       |
...
|        | E2PD3    |
|--------|----------|
| [7:0]  | В 32-битном режиме: хранит старшие 8 бит старших 16 бит данных. |

### ECCR - Регистр управления

| ECCR: 0x36 (0x56)       | Значение по умолчанию: 0x0C |
|-------------------------|------------------------------|
...
**[7] WEN**  
Защита записи ECCR  
Перед изменением ECCR необходимо сначала записать 1 в WEN, а затем обновить содержимое регистра ECCR в течение 6 системных тактов.

**[6] EEN**  
Разрешение E2PROM (действительно только для LGT8F328P)  
- **1:** Включена эмуляция E2PROM, часть пространства 32 КБ FLASH будет зарезервирована.  
- **0:** Эмуляция E2PROM отключена, все 32 КБ FLASH используются только для программного пространства.

**[5] ERN**  
Запись 1 сбрасывает контроллер E2PCTL.

**[4] SWM**  
Режим непрерывной записи (действует при операциях с эмулируемым контроллером E2PROM).

**[3] CP1**  
Управление разрешением обмена областью CP1 при страничном обмене.

**[2] CP0**  
Управление разрешением обмена областью CP0 при страничном обмене.

**[1:0] ECS[1:0]**  
Конфигурация пространства E2PROM:  

| ECS[1:0] | E2PROM      | Программная FLASH |
|----------|-------------|-------------------|
| 00       | 1 КБ        | 30 КБ             |
| 01       | 2 КБ        | 28 КБ             |
| 10       | 4 КБ        | 24 КБ             |
...
| EECR: 0x1F (0x3F)           | Значение по умолчанию: 0x00 |
|-----------------------------|------------------------------|
| Биты                        | EEPM3 | EEPM2 | EEPM1 | EEPM0 | EERIE | EEMPE | EEPE | EERE |
| R/W                         | R/W   | R/W   | R/W   | R/W   | R/W   | R/W   | R/W  | R/W  |
...
#### [7:4] — **EEPM[3:0]**
Биты управления режимом доступа к EFLASH/E2PROM
...
#### [3] — **EERIE**
Управление прерыванием готовности FLASH/E2PROM.  
- **1** — разрешено, **0** — запрещено.  
Прерывание готовности E2PROM возникает после того, как бит EEPE аппаратно сброшен в 0. Во время операций с EPROM (программной FLASH) это прерывание не генерируется.

#### [2] — **EEMPE**
Бит разрешения операции программирования FLASH/E2PROM.  
Используется для управления тем, активен ли бит EEPE.  
Если установить EEMPE = 1 (при EEPE = 0), то в течение следующих четырёх тактов установка EEPE = 1 запустит операцию программирования. В противном случае операция программирования не будет выполнена. Через четыре такта EEMPE автоматически сбрасывается в 0.

#### [1] — **EEPE**
Бит разрешения операции программирования FLASH/E2PROM (непосредственный запуск).

#### [0] — **EERE**
Бит разрешения чтения E2PROM. Данные становятся действительными через два системных такта после установки этого бита.

### GPIOR2 - Регистр ввода-вывода 2

|                        | GPIO2R — Регистр ввода-вывода общего назначения 2 |
|------------------------|----------------------------------------------------|
| **GPIO2R: 0x2B (0x4B)** | Значение по умолчанию: 0x00                       |
| Биты                   | GPIO2R[7:0]                                       |
| R/W                    | R/W                                                |
| Начальное значение     | 0x00                                               |

#### Описание битов
**[7:0] GPIO2R** — регистр ввода-вывода общего назначения 2, предназначен для хранения пользовательских данных.

---

### GPIOR1 - Регистр ввода-вывода 1

|                         | GPIO1R1 — Регистр ввода-вывода общего назначения 1 |
|-------------------------|-----------------------------------------------------|
| **GPIO1R1: 0x2A (0x4A)** | Значение по умолчанию: 0x00                        |
| Биты                    | GPIO1R1[7:0]                                       |
| R/W                     | R/W                                                 |
| Начальное значение      | 0x00                                                |

#### Описание битов
**[7:0] GPIO1R1** — регистр ввода-вывода общего назначения 1, предназначен для хранения пользовательских данных.

---

### GPIOR0 - Регистр ввода-вывода 0

|                         | GPIO0R0 — Регистр ввода-вывода общего назначения 0 |
|-------------------------|-----------------------------------------------------|
| **GPIO0R0: 0x1E (0x3E)** | Значение по умолчанию: 0x00                        |
| Биты                    | GPIO0R0[7:0]                                       |
| R/W                     | R/W                                                 |
| Начальное значение      | 0x00                                                |

#### Описание битов
**[7:0] GPIO0R0** — регистр ввода-вывода общего назначения 0, предназначен для хранения пользовательских данных.