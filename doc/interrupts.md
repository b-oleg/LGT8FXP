...
| № | Адрес вектора | Сигнал источника прерывания | Описание источника прерывания |
|-|-|-|-|
...
### Пример кода на ассемблере для LGT8F88P
...
|-----|-------------|----------|
| [0] | WCE | Бит разрешения обновления MCUCR. Перед обновлением MCUCR необходимо установить этот бит, затем выполнить обновление регистра MCUCR в течение 6 циклов. |
| [1] | IVSEL | Бит выбора вектора прерывания. Когда этот бит установлен в 1, адреса векторов прерываний будут переназначены на новый адрес в соответствии со значением регистра IVBASE. |
...
| [7] | FWKEN | Управление режимом быстрого пробуждения. Действителен только для режима Power/Off. <br>0 = задержка фильтрации 260 мкс <br>1 = задержка фильтрации 32 мкс |
...
|-----|-----|----------|
| 7:4 | - | Зарезервировано. |
| 3 | ISC11 | Старший бит управления режимом触发 прерывания для вывода INT1. |
| 2 | ISC10 | Младший бит управления режимом触发 прерывания для вывода INT1. <br><br>Когда глобальные прерывания разрешены и установлен соответствующий бит маски прерывания в регистре GICR, внешнее прерывание 1 генерируется выводом INT1. Режим触发 прерывания описан в таблице. Перед обнаружением фронта MCU сначала производит выборку уровня на выводе INT1. Если выбран режим触发 по фронту или по изменению уровня, импульс длительностью более 1 такта системного时钟 вызовет прерывание; слишком короткие импульсы не гарантируют срабатывания прерывания. Если выбран режим触发 по низкому уровню, то низкий уровень должен сохраняться до завершения выполнения текущей инструкции, чтобы вызвать прерывание. |
| 1 | ISC01 | Старший бит управления режимом触发 прерывания для вывода INT0. |
| 0 | ISC00 | Младший бит управления режимом触发 прерывания для вывода INT0. <br><br>Когда глобальные прерывания разрешены и установлен соответствующий бит маски прерывания в регистре GICR, внешнее прерывание 0 генерируется выводом INT0. Режим触发 прерывания описан в таблице. Перед обнаружением фронта MCU сначала производит выборку уровня на выводе INT0. Если выбран режим触发 по фронту или по изменению уровня, импульс длительностью более 1 такта системного时钟 вызовет прерывание; слишком короткие импульсы не гарантируют срабатывания прерывания. Если выбран режим触发 по низкому уровню, то низкий уровень должен сохраняться до завершения выполнения текущей инструкции, чтобы вызвать прерывание. |

---

*Примечание: В оригинальном документе на страницах 49 указана информация о режимах срабатывания (таблица), которая здесь не приведена, но упоминается в тексте.*